...
class PredictionsAnalyzer():
    ...
    def __init__(self, task):
        ...
        if task=="pos":
            ...
            self.unique_tags = "B-LOC B-ORG B-PER I-LOC I-ORG I-PER O".split()
        else:
            raise ValueError("Wrong task name. Possible task names are \"pos\" or \"ner\"")
...
#         remove empty lines at the end
        self.all_sentences = [value for value in self.all_sentences if value!=""]
        self.all_tags = [value for value in self.all_tags if value!=""]
    ...
    #     tags satisfying the condition
        ...
        for i in range(len(self.all_tags)):
            if len(self.all_tags[i].split())==len(self.all_sentences[i].split()):
